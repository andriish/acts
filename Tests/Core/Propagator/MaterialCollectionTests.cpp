...
std::cout << "params: " << pT << " " << phi << " " << theta << " " << charge << " " << time << " " << index << std::endl;
  if (index < skip) {
    return;
  }
  ...
  BoundMatrix covycov = BoundMatrix::Identity();
  CurvilinearParameters start(covycov, pos, mom, q, time);
  ...
  fwdMaterialInteractor.energyLoss = true;
  ...
  std::cout << *(fwdResult.endParameters->covariance()) << std::endl;
