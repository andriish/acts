...
<<<<<<< HEAD
  BOOST_CHECK_NE(std::get<BoundMatrix>(covariance), BoundSymMatrix::Identity());
...
=======
  ...
  BOOST_TEST(std::get<BoundMatrix>(jacobian) !=
             BoundMatrix(2. * BoundMatrix::Identity()));
...
>>>>>>> Extended CovarianceEngineTests
  ...
  BOOST_CHECK_NE(*(std::get<0>(curvResult).covariance()),
                 BoundSymMatrix::Identity());
  BOOST_REQUIRE_NO_THROW(std::get<BoundMatrix>(std::get<1>(curvResult)));
