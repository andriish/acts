...
<<<<<<< HEAD
  ...
  if constexpr(return_parameters_t::is_local_representation && parameters_t::is_local_representation)
  {
	  ...
	  std::array<std::vector<BoundVector>, eBoundParametersSize> derivatives; // TODO: This requires different dimensions
...
>>>>>>> Ridders cov calculation extended
        }
      }
    }
    mParSet->setCovariance(
...
Acts::RiddersPropagator<propagator_t>::wiggleDimension(
  ...
  for (double h : deviations) {
    parameters_t tp = wiggleStartVector(options.geoContext, h, param, startPars);
...
parameters_t
Acts::RiddersPropagator<propagator_t>::wiggleStartVector(std::reference_wrapper<const GeometryContext> geoContext, double h, const unsigned int param, parameters_t tp) const
{
	if constexpr (parameters_t::is_local_representation)
    {
...
template<typename parameters_t>
void
Acts::RiddersPropagator<propagator_t>::wiggleBoundStartVector(std::reference_wrapper<const GeometryContext> geoContext, double h, const unsigned int param, parameters_t& tp) const
{
	// Treatment for theta
	if (param == eTHETA) {
	...
	switch (param) {
	  case 0: {
		tp.template set<eLOC_0>(geoContext,
								tp.template get<eLOC_0>() + h);
		break;
	  }
	  case 1: {
		tp.template set<eLOC_1>(geoContext,
	  ...
	  case 3: {
		tp.template set<eTHETA>(geoContext,
...
template<typename parameters_t>
void
Acts::RiddersPropagator<propagator_t>::wiggleFreeStartVector(std::reference_wrapper<const GeometryContext> geoContext, double h, const unsigned int param, parameters_t& tp) const
{
	  switch (param) {
	  case 0: {
		tp.template set<0>(geoContext,
								tp.template get<0>() + h);
		break;
	  }
	  case 1: {
		tp.template set<1>(geoContext,
								tp.template get<1>() + h);
		break;
	  }
	  case 2: {
		tp.template set<2>(geoContext,
								tp.template get<2>() + h);
		break;
	  }
	  case 3: {
		tp.template set<3>(geoContext,
								tp.template get<3>() + h);
		break;
	  }
	  case 4: {
		tp.template set<4>(geoContext,
								tp.template get<4>() + h);
		break;
	  }
	  case 5: {
		tp.template set<5>(geoContext,
								tp.template get<5>() + h);
		break;
	  }
	  case 6: {
		tp.template set<6>(geoContext,
								tp.template get<6>() + h);
		break;
	  }
	  case 7: {
		tp.template set<7>(geoContext,
								tp.template get<7>() + h);
		break;
	  }
	}
...
auto Acts::RiddersPropagator<propagator_t>::calculateCovariance(
    const std::array<std::vector<BoundVector>, Acts::eBoundParametersSize>& derivatives,
...
template <typename propagator_t>
auto Acts::RiddersPropagator<propagator_t>::calculateCovariance(
    const std::array<std::vector<BoundVector>, Acts::eFreeParametersSize>& derivatives,
    const std::variant<Acts::BoundSymMatrix, Acts::FreeSymMatrix>& startCov,
    const std::vector<double>& deviations) const -> const Covariance {
...
template <typename propagator_t>
auto Acts::RiddersPropagator<propagator_t>::calculateCovariance(
    const std::array<std::vector<FreeVector>, Acts::eBoundParametersSize>& derivatives,
    const std::variant<Acts::BoundSymMatrix, Acts::FreeSymMatrix>& startCov,
    const std::vector<double>& deviations) const -> const Covariance {
...
template <typename propagator_t>
auto Acts::RiddersPropagator<propagator_t>::calculateCovariance(
    const std::array<std::vector<FreeVector>, Acts::eFreeParametersSize>& derivatives,
    const std::variant<Acts::BoundSymMatrix, Acts::FreeSymMatrix>& startCov,
    const std::vector<double>& deviations) const -> const Covariance {
  FreeMatrix jacobian;
  jacobian.setIdentity();
