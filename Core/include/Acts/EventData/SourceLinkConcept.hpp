...
namespace concept {
  ...
  template <typename T>
  using object_method_t = decltype(std::declval<T>().referenceObject());
  ...
  struct SourceLinkConcept {
    ...
    constexpr static bool object_method_exists =
        converts_to<const GeometryObject&, object_method_t, T>;
    static_assert(
        object_method_exists,
        "Source link does not have compliant referenceObject method");
    ...
    constexpr static bool value =
        concept ::require<comparison_works, object_method_exists, copyable,
