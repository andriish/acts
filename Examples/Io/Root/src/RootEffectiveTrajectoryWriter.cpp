...
using Measurement = std::variant<Acts::Measurement<FW::EffectiveSourceLink, Acts::BoundParametersIndices, Acts::BoundParametersIndices::eLOC_0>,
						  Acts::Measurement<FW::EffectiveSourceLink, Acts::BoundParametersIndices,Acts::BoundParametersIndices::eLOC_0, Acts::BoundParametersIndices::eLOC_1>, 
						  Acts::Measurement<FW::EffectiveSourceLink, Acts::FreeParametersIndices, Acts::FreeParametersIndices::eFreePos0, Acts::FreeParametersIndices::eFreePos1, Acts::FreeParametersIndices::eFreePos2>>;

FW::RootEffectiveTrajectoryWriter::RootEffectiveTrajectoryWriter(
    const FW::RootEffectiveTrajectoryWriter::Config& cfg, Acts::Logging::Level lvl)
    : WriterT(cfg.inputTrajectories, "RootEffectiveTrajectoryWriter", lvl),
      m_cfg(cfg),
...
FW::ProcessCode FW::RootEffectiveTrajectoryWriter::endRun() {
  if (m_outputFile) {
    m_outputFile->cd();
    m_outputTree->Write();
...
FW::ProcessCode FW::RootEffectiveTrajectoryWriter::writeT(
    const AlgorithmContext& ctx, const std::vector<EffectiveMultiTrajectory>& trajectories) {
  if (m_outputFile == nullptr)
    return ProcessCode::SUCCESS;
  ...
  for (const auto& traj : trajectories) {
    ...
    mj.visitBackwards(trackTip, [&](const auto& state) {
      ...
      auto measVariant = *state.uncalibrated();
      if(auto meas = std::get_if<1>(&measVariant))
      {
		  // get the Surface
...
const auto sigmac = (covInv + covInvPrt).inverse();
const auto measParams = meas->parameters();
const auto mc1 = sigmac * covInv * measParams;
const auto mc2 = sigmac * covInvPrt * parameters.template segment<2>(Acts::eBoundLoc0);
const auto mc = mc1 + mc2;

			Acts::ActsSymMatrixD<2> resCov = cov + H * covariance * H.transpose();
			auto residual = meas->residual(parameters);
			
			//~ Eigen::EigenSolver<Acts::ActsSymMatrixD<2>> es(resCov);
			//~ Eigen::EigenSolver<Acts::ActsSymMatrixD<2>> es(sigmac);
			//~ Acts::ActsSymMatrixD<2> eVal = Eigen::Matrix2cd(es.eigenvalues().asDiagonal()).real();
			//~ auto eVec = es.eigenvectors().real().colwise().normalized();
			
			Acts::Vector2D trth{truthLOC0, truthLOC1};
			//~ residual = mc - trth;
			//~ resCov = sigmac;
			
			Eigen::EigenSolver<Acts::ActsSymMatrixD<2>> es(sigmac);
			Acts::ActsSymMatrixD<2> eVal = Eigen::Matrix2cd(es.eigenvalues().asDiagonal()).real();
			auto eVec = es.eigenvectors().real();

			residual = eVec.inverse() * (mc - trth);
			resCov = eVal;
...
//~ std::cout << "Offdiagonal entires: " << covariance(Acts::eFreePos0, Acts::eBoundLoc1) << " | " << m_pull_eLOC0_prt.back() << std::endl;
		  ...
		  if (state.hasBoundFiltered()) {
			...
			// local hit residual info
			auto H = meas->projector();
const Acts::ActsSymMatrixD<2> covInv = cov.inverse();			
const Acts::ActsSymMatrixD<2> covPrt = H * covariance * H.transpose();
const Acts::ActsSymMatrixD<2> covInvPrt = covPrt.inverse();
const Acts::ActsSymMatrixD<2> sigmac = (covInv + covInvPrt).inverse();
const Acts::Vector2D measParams = meas->parameters();
const Acts::Vector2D mc1 = sigmac * covInv * measParams;
const Acts::Vector2D mc2 = sigmac * covInvPrt * parameters.template segment<2>(Acts::eBoundLoc0);
const Acts::Vector2D mc = mc1 + mc2;
//~ std::cout << "Covariance:\n:" << covPrt << std::endl;
			...
			m_pull_eT_flt.push_back(
				(parameters[Acts::eBoundTime] - truthTIME) /
				sqrt(covariance(Acts::eBoundTime, Acts::eBoundTime)));
//~ std::cout << "Pull: " << m_pull_eLOC0_flt.back() << " | " << m_pull_eLOC1_flt.back() << std::endl;	
...
const auto covPrt = H * covariance * H.transpose();
const auto covInvPrt = covPrt.inverse();
Acts::ActsSymMatrixD<2> sigmac = (covInv + covInvPrt).inverse();
const auto measParams = meas->parameters();
const auto mc1 = sigmac * covInv * measParams;
const auto mc2 = sigmac * covInvPrt * parameters.template segment<2>(Acts::eBoundLoc0);
Acts::Vector2D mc = mc1 + mc2;

			Acts::ActsSymMatrixD<2> resCov = cov + H * covariance * H.transpose();
			auto residual = meas->residual(parameters);
			
			mc = parameters.template segment<2>(Acts::eBoundLoc0);
			sigmac = H * covariance * H.transpose();
...
//~ std::cout << "truth: " << trth.transpose() << " | " << (mc - trth).transpose() << " | " << (eVec * (mc - trth)).transpose()
	//~ << " | " << (eVec.inverse() * (mc - trth)).transpose() << std::endl;
//~ std::cout << "sigmac:\n" << sigmac << std::endl;		
//~ std::cout << "eVal:\n" << eVal << std::endl << sqrt(eVal(0, 0)) << " | " << sqrt(eVal(1, 1)) << " | " << sqrt(eVal(2, 2)) << std::endl;		
			...
			m_pull_eT_smt.push_back(
				(parameters[Acts::eBoundTime] - truthTIME) /
				sqrt(covariance(Acts::eBoundTime, Acts::eBoundTime)));
//~ std::cout << "Pull: " << m_pull_eLOC0_smt.back() << " | " << m_pull_eLOC1_smt.back() << std::endl;
...
std::cout << "Position: " << freePosition.transpose() << " | " << state.pathLength() << std::endl;
...
//~ }
	  ...
	  {
	  if(auto meas = std::get_if<2>(&measVariant))
      {
		  // get the Volume
		  const Acts::Volume* volumePtr =
			  dynamic_cast<const Acts::Volume*>(&meas->referenceObject());
		  ...
		  m_t_dy.push_back(truthHit.unitDirection().y());
		  m_t_dz.push_back(truthHit.unitDirection().z());
		  m_t_ePHI.push_back(phi(truthHit.unitDirection()));
		  m_t_eTHETA.push_back(theta(truthHit.unitDirection()));
		  m_t_eQOP.push_back(m_t_charge / truthHit.momentum4Before().template head<3>().norm());
		  m_t_eT.push_back(truthHit.time());
...
const Acts::Vector3D mc = mc1 + mc2;
			...
			//~ Acts::ActsSymMatrixD<3> eVal = Eigen::Matrix3cd(es.eigenvalues().asDiagonal()).real();
			//~ auto eVec = es.eigenvectors().real().colwise().normalized();
...
//~ std::cout << "sigmac:\n" << sigmac << std::endl;		
//~ std::cout << "eVal:\n" << eVal << std::endl << sqrt(eVal(0, 0)) << " | " << sqrt(eVal(1, 1)) << " | " << sqrt(eVal(2, 2)) << std::endl;		
			m_res_x_hit.push_back(residual.x());
			m_res_y_hit.push_back(residual.y());
			m_res_z_hit.push_back(residual.z());
			m_err_x_hit.push_back(sqrt(resCov(Acts::eFreePos0, Acts::eFreePos0)));
			m_err_y_hit.push_back(sqrt(resCov(Acts::eFreePos1, Acts::eFreePos1)));
			m_err_z_hit.push_back(sqrt(resCov(Acts::eFreePos2, Acts::eFreePos2)));

			m_pull_x_hit.push_back(
				residual(Acts::eFreePos0) /
				sqrt(resCov(Acts::eFreePos0, Acts::eFreePos0)));
			m_pull_y_hit.push_back(
				residual(Acts::eFreePos1) /
				sqrt(resCov(Acts::eFreePos1, Acts::eFreePos1)));
			m_pull_z_hit.push_back(
				residual(Acts::eFreePos2) /
				sqrt(resCov(Acts::eFreePos2, Acts::eFreePos2)));
			m_dim_hit.push_back(state.calibratedSize());

			// predicted parameter
			m_ePos0_prt.push_back(parameters[Acts::eFreePos0]);
			m_ePos1_prt.push_back(parameters[Acts::eFreePos1]);
			m_ePos2_prt.push_back(parameters[Acts::eFreePos2]);
			m_eT_prt.push_back(parameters[Acts::eFreeTime]);
			m_eDir0_prt.push_back(parameters[Acts::eFreeDir0]);
			m_eDir1_prt.push_back(parameters[Acts::eFreeDir1]);
			m_eDir2_prt.push_back(parameters[Acts::eFreeDir2]);
			m_eQOP_prt.push_back(parameters[Acts::eFreeQOverP]);
			
			Acts::Vector3D momentum;
			momentum = parameters.template segment<3>(Acts::eFreeDir0);
			momentum /= std::abs(parameters[Acts::eFreeQOverP]);
			m_px_prt.push_back(momentum.x());
			m_py_prt.push_back(momentum.y());
			m_pz_prt.push_back(momentum.z());
			m_pT_prt.push_back(perp(momentum));
			m_eta_prt.push_back(eta(parameters.template segment<3>(Acts::eFreePos0)));
			m_x_prt.push_back(parameters[Acts::eFreePos0]);
			m_y_prt.push_back(parameters[Acts::eFreePos1]);
			m_z_prt.push_back(parameters[Acts::eFreePos2]);
						
			// predicted residual
			m_res_ePos0_prt.push_back(parameters[Acts::eFreePos0] - truthHit.position().x());
			m_res_ePos1_prt.push_back(parameters[Acts::eFreePos1] - truthHit.position().y());
			m_res_ePos2_prt.push_back(parameters[Acts::eFreePos2] - truthHit.position().z());
			m_res_eT_prt.push_back(parameters[Acts::eFreeTime] - truthHit.time());
			m_res_eDir0_prt.push_back(parameters[Acts::eFreeDir0] - truthHit.unitDirection().x());
			m_res_eDir1_prt.push_back(parameters[Acts::eFreeDir1] - truthHit.unitDirection().y());
			m_res_eDir2_prt.push_back(parameters[Acts::eFreeDir2] - truthHit.unitDirection().z());
			m_res_eQOP_prt.push_back(parameters[Acts::eFreeQOverP] - m_t_charge / truthHit.momentum4Before().template head<3>().norm());

			// predicted parameter error
			m_err_ePos0_prt.push_back(
				sqrt(covariance(Acts::eFreePos0, Acts::eFreePos0)));
			m_err_ePos1_prt.push_back(
				sqrt(covariance(Acts::eFreePos1, Acts::eFreePos1)));
			m_err_ePos2_prt.push_back(
				sqrt(covariance(Acts::eFreePos2, Acts::eFreePos2)));
			m_err_eT_prt.push_back(
				sqrt(covariance(Acts::eFreeTime, Acts::eFreeTime)));
			m_err_eDir0_prt.push_back(
				sqrt(covariance(Acts::eFreeDir0, Acts::eFreeDir0)));
			m_err_eDir1_prt.push_back(
				sqrt(covariance(Acts::eFreeDir1, Acts::eFreeDir1)));
			m_err_eDir2_prt.push_back(
			...
			//~ m_pull_ePos2_prt.push_back((parameters[Acts::eFreePos2] - truthHit.position().z())/
				//~ sqrt(covariance(Acts::eFreePos2, Acts::eFreePos2)));
			m_pull_ePos0_prt.push_back((residual.x())/
				sqrt(resCov(Acts::eFreePos0, Acts::eFreePos0)));
			m_pull_ePos1_prt.push_back((residual.y())/
				sqrt(resCov(Acts::eFreePos1, Acts::eFreePos1)));
...
//~ std::cout << "Pull Pos0 Free Prt: " << state.index() << ", " << parameters[Acts::eFreePos0] << " " << parameters[Acts::eFreePos1] << " " << parameters[Acts::eFreePos2]
	...
	//~ << " | " << m_pull_ePos0_prt.back() << std::endl;
			...
			m_pull_eDir0_prt.push_back((parameters[Acts::eFreeDir0] - truthHit.unitDirection().x())/
				sqrt(covariance(Acts::eFreeDir0, Acts::eFreeDir0)));
			m_pull_eDir1_prt.push_back((parameters[Acts::eFreeDir1] - truthHit.unitDirection().y())/
				sqrt(covariance(Acts::eFreeDir1, Acts::eFreeDir1)));
			m_pull_eDir2_prt.push_back((parameters[Acts::eFreeDir2] - truthHit.unitDirection().z())/
				sqrt(covariance(Acts::eFreeDir2, Acts::eFreeDir2)));
			m_pull_eQOP_prt.push_back((parameters[Acts::eFreeQOverP] - m_t_charge / truthHit.momentum4Before().template head<3>().norm())/
				sqrt(covariance(Acts::eFreeQOverP, Acts::eFreeQOverP)));
		  } else {
			// push default values if no predicted parameter
			m_res_x_hit.push_back(-99.);
			m_res_y_hit.push_back(-99.);
			m_res_z_hit.push_back(-99.);
			m_err_x_hit.push_back(-99.);
			m_err_y_hit.push_back(-99.);
			m_err_z_hit.push_back(-99.);
			m_pull_x_hit.push_back(-99.);
			m_pull_y_hit.push_back(-99.);
			m_pull_z_hit.push_back(-99.);
			m_dim_hit.push_back(-99.);
			m_ePos0_prt.push_back(-99.);
			m_ePos1_prt.push_back(-99.);
			m_ePos2_prt.push_back(-99.);
			m_eT_prt.push_back(-99.);
			m_eDir0_prt.push_back(-99.);
			m_eDir1_prt.push_back(-99.);
			m_eDir2_prt.push_back(-99.);
			m_eQOP_prt.push_back(-99.);
			m_res_ePos0_prt.push_back(-99.);
			m_res_ePos1_prt.push_back(-99.);
			m_res_ePos2_prt.push_back(-99.);
			m_res_eT_prt.push_back(-99.);
			m_res_eDir0_prt.push_back(-99.);
			m_res_eDir1_prt.push_back(-99.);
			m_res_eDir2_prt.push_back(-99.);
			m_res_eQOP_prt.push_back(-99.);
			m_err_ePos0_prt.push_back(-99);
			m_err_ePos1_prt.push_back(-99);
			m_err_ePos2_prt.push_back(-99);
			m_err_eT_prt.push_back(-99);
			m_err_eDir0_prt.push_back(-99);
			m_err_eDir1_prt.push_back(-99);
			m_err_eDir2_prt.push_back(-99);
			m_err_eQOP_prt.push_back(-99);
			//~ m_pull_ePos0_prt.push_back(-99.);
			//~ m_pull_ePos1_prt.push_back(-99.);
			//~ m_pull_ePos2_prt.push_back(-99.);
			m_pull_eT_prt.push_back(-99.);
			//~ m_pull_eDir0_prt.push_back(-99.);
			//~ m_pull_eDir1_prt.push_back(-99.);
			//~ m_pull_eDir2_prt.push_back(-99.);
			m_pull_eQOP_prt.push_back(-99.);
			m_x_prt.push_back(-99.);
			m_y_prt.push_back(-99.);
			m_z_prt.push_back(-99.);
		  ...
		  //~ if (state.hasFreeFiltered() && state.hasJacobianBoundToFree()) {
			...
			// local hit residual info
			Acts::ActsMatrixD<3, 8> H = meas->projector();
Acts::Vector3D dir = parameters.template segment<3>(Acts::eFreeDir0);
Acts::ActsSymMatrixD<3> dirMat2 = dir * dir.transpose() + covariance0.template block<3, 3>(Acts::eFreeDir0, Acts::eFreeDir0);
double v = cov.diagonal().sum();
const Acts::ActsSymMatrixD<8> B = H.transpose() * v * dirMat2 * H;

const Acts::ActsMatrixD<8, 3> K = (state.freePredictedCovariance() + B) * H.transpose() * (H * (state.freePredictedCovariance() + B) * H.transpose() + cov).inverse();
const Acts::ActsSymMatrixD<8> covariance = covariance0
										   ...
										   + B - K * H * B - B * H.transpose() * K.transpose();
										   //~ + B
										   //~ - B * H.transpose() * K.transpose()
										   //~ - K * H * B;
//~ std::cout << "B\n" << B << std::endl;
//~ std::cout << "KHB\n" << K * H * B << std::endl;
//~ std::cout << "BHK\n" << B * H.transpose() * K.transpose() << std::endl;
//~ const Acts::ActsSymMatrixD<3> covInv = cov.inverse();			
Acts::ActsSymMatrixD<3> covPrt = H * covariance * H.transpose();
//~ const Acts::ActsSymMatrixD<3> covInvPrt = covPrt.inverse();
Acts::ActsSymMatrixD<3> sigmac;// = (covInv + covInvPrt).inverse();
...
//~ const Acts::Vector3D mc1 = sigmac * covInv * measParams;
//~ const Acts::Vector3D mc2 = sigmac * covInvPrt * parameters.template segment<3>(Acts::eFreePos0);
Acts::Vector3D mc;// = mc1 + mc2;
			Acts::ActsSymMatrixD<3> resCov;// = cov + H * covariance * H.transpose();
			Acts::Vector3D residual;// = meas->residual(parameters);


			sigmac = covPrt;
			mc = parameters.template segment<3>(Acts::eFreePos0);
...
//~ std::cout << "covariance:\n" << covPrt << std::endl;
			...
			m_ePos2_flt.push_back(parameters[Acts::eFreePos2]);
			m_eT_flt.push_back(parameters[Acts::eFreeTime]);
			m_eDir0_flt.push_back(parameters[Acts::eFreeDir0]);
			m_eDir1_flt.push_back(parameters[Acts::eFreeDir1]);
			m_eDir2_flt.push_back(parameters[Acts::eFreeDir2]);
			m_eQOP_flt.push_back(parameters[Acts::eFreeQOverP]);
			
			Acts::Vector3D momentum;
			momentum = parameters.template segment<3>(Acts::eFreeDir0);
			momentum /= std::abs(parameters[Acts::eFreeQOverP]);
			m_px_flt.push_back(momentum.x());
			m_py_flt.push_back(momentum.y());
			m_pz_flt.push_back(momentum.z());
			m_pT_flt.push_back(perp(momentum));
			m_eta_flt.push_back(eta(parameters.template segment<3>(Acts::eFreePos0)));
			m_x_flt.push_back(parameters[Acts::eFreePos0]);
			m_y_flt.push_back(parameters[Acts::eFreePos1]);
			m_z_flt.push_back(parameters[Acts::eFreePos2]);
						
			// Filtered residual
			m_res_ePos0_flt.push_back(parameters[Acts::eFreePos0] - truthHit.position().x());
			m_res_ePos1_flt.push_back(parameters[Acts::eFreePos1] - truthHit.position().y());
			m_res_ePos2_flt.push_back(parameters[Acts::eFreePos2] - truthHit.position().z());
			m_res_eT_flt.push_back(parameters[Acts::eFreeTime] - truthHit.time());
			m_res_eDir0_flt.push_back(parameters[Acts::eFreeDir0] - truthHit.unitDirection().x());
			m_res_eDir1_flt.push_back(parameters[Acts::eFreeDir1] - truthHit.unitDirection().y());
			m_res_eDir2_flt.push_back(parameters[Acts::eFreeDir2] - truthHit.unitDirection().z());
			m_res_eQOP_flt.push_back(parameters[Acts::eFreeQOverP] - m_t_charge / truthHit.momentum4Before().template head<3>().norm());

			// Filtered parameter error
			m_err_ePos0_flt.push_back(
				sqrt(covariance(Acts::eFreePos0, Acts::eFreePos0)));
			m_err_ePos1_flt.push_back(
				sqrt(covariance(Acts::eFreePos1, Acts::eFreePos1)));
			m_err_ePos2_flt.push_back(
				sqrt(covariance(Acts::eFreePos2, Acts::eFreePos2)));
			m_err_eT_flt.push_back(
				sqrt(covariance(Acts::eFreeTime, Acts::eFreeTime)));
			m_err_eDir0_flt.push_back(
				sqrt(covariance(Acts::eFreeDir0, Acts::eFreeDir0)));
			m_err_eDir1_flt.push_back(
				sqrt(covariance(Acts::eFreeDir1, Acts::eFreeDir1)));
			m_err_eDir2_flt.push_back(
				sqrt(covariance(Acts::eFreeDir2, Acts::eFreeDir2)));
			m_err_eQOP_flt.push_back(
				sqrt(covariance(Acts::eFreeQOverP, Acts::eFreeQOverP)));

			// Filtered parameter pull
			//~ m_pull_ePos0_flt.push_back((parameters[Acts::eFreePos0] - truthHit.position().x())/
				//~ sqrt(covariance(Acts::eFreePos0, Acts::eFreePos0)));
			//~ m_pull_ePos1_flt.push_back((parameters[Acts::eFreePos1] - truthHit.position().y())/
				//~ sqrt(covariance(Acts::eFreePos1, Acts::eFreePos1)));
			...
			m_pull_eT_flt.push_back((parameters[Acts::eFreeTime] - truthHit.time())/
				sqrt(covariance(Acts::eFreeTime, Acts::eFreeTime)));
			m_pull_eDir0_flt.push_back((parameters[Acts::eFreeDir0] - truthHit.unitDirection().x())/
				sqrt(covariance(Acts::eFreeDir0, Acts::eFreeDir0)));
			m_pull_eDir1_flt.push_back((parameters[Acts::eFreeDir1] - truthHit.unitDirection().y())/
				sqrt(covariance(Acts::eFreeDir1, Acts::eFreeDir1)));
			m_pull_eDir2_flt.push_back((parameters[Acts::eFreeDir2] - truthHit.unitDirection().z())/
				sqrt(covariance(Acts::eFreeDir2, Acts::eFreeDir2)));
...
//~ std::cout << "Pull Pos0 Free Flt: " << state.index() << ", " << parameters[Acts::eFreePos0] << " " << parameters[Acts::eFreePos1] << " " << parameters[Acts::eFreePos2]
	...
	//~ << sqrt(covariance(Acts::eFreePos0, Acts::eFreePos0)) << " | " << m_pull_ePos0_flt.back() << ", " << m_pull_ePos1_flt.back() << ", " << m_pull_ePos2_flt.back() << std::endl;
			...
			// push default values if no filtered parameter
			m_ePos0_flt.push_back(-99.);
			m_ePos1_flt.push_back(-99.);
			m_ePos2_flt.push_back(-99.);
			m_eT_flt.push_back(-99.);
			m_eDir0_flt.push_back(-99.);
			...
			m_err_eDir2_flt.push_back(-99);
			m_err_eQOP_flt.push_back(-99);
			//~ m_pull_ePos0_flt.push_back(-99.);
			//~ m_pull_ePos1_flt.push_back(-99.);
			//~ m_pull_ePos2_flt.push_back(-99.);
			m_pull_eT_flt.push_back(-99.);
			//~ m_pull_eDir0_flt.push_back(-99.);
			//~ m_pull_eDir1_flt.push_back(-99.);
			//~ m_pull_eDir2_flt.push_back(-99.);
			m_pull_eQOP_flt.push_back(-99.);
			m_x_flt.push_back(-99.);
			m_y_flt.push_back(-99.);
			m_z_flt.push_back(-99.);
		  ...
		  // get the smoothed parameter
		  bool smoothed = false;
		  if (state.hasFreeSmoothed()) {// && false) {
			smoothed = true;
			//~ m_nSmoothed++;
...
const Acts::ActsMatrixD<8, 3> K = (state.freePredictedCovariance() + B) * H.transpose() * (H * (state.freePredictedCovariance() + B) * H.transpose() + cov).inverse();
const Acts::ActsSymMatrixD<8> covariance = covariance0 + B - K * H * B - B * H.transpose() * K.transpose();
										   //~ + (Acts::ActsSymMatrixD<8>::Identity() - K * H) * B 
										   //~ - B * (Acts::ActsSymMatrixD<8>::Identity() - K * H).transpose() 
										   //~ + 2. * B;
...
Acts::Vector3D mc;// = mc1 + mc2;
			...
			// Smoothed parameter
			m_ePos0_smt.push_back(parameters[Acts::eFreePos0]);
			m_ePos1_smt.push_back(parameters[Acts::eFreePos1]);
			m_ePos2_smt.push_back(parameters[Acts::eFreePos2]);
			m_eT_smt.push_back(parameters[Acts::eFreeTime]);
			...
			momentum = parameters.template segment<3>(Acts::eFreeDir0);
			momentum /= std::abs(parameters[Acts::eFreeQOverP]);
			m_px_smt.push_back(momentum.x());
			m_py_smt.push_back(momentum.y());
			m_pz_smt.push_back(momentum.z());
			m_pT_smt.push_back(perp(momentum));
			m_eta_smt.push_back(eta(parameters.template segment<3>(Acts::eFreePos0)));
			m_x_smt.push_back(parameters[Acts::eFreePos0]);
			m_y_smt.push_back(parameters[Acts::eFreePos1]);
			m_z_smt.push_back(parameters[Acts::eFreePos2]);

			// Smoothed residual
			m_res_ePos0_smt.push_back(parameters[Acts::eFreePos0] - truthHit.position().x());
			m_res_ePos1_smt.push_back(parameters[Acts::eFreePos1] - truthHit.position().y());
			m_res_ePos2_smt.push_back(parameters[Acts::eFreePos2] - truthHit.position().z());
			m_res_eT_smt.push_back(parameters[Acts::eFreeTime] - truthHit.time());
			...
			m_err_ePos0_smt.push_back(
				sqrt(covariance(Acts::eFreePos0, Acts::eFreePos0)));
			m_err_ePos1_smt.push_back(
				sqrt(covariance(Acts::eFreePos1, Acts::eFreePos1)));
			m_err_ePos2_smt.push_back(
				sqrt(covariance(Acts::eFreePos2, Acts::eFreePos2)));
			m_err_eT_smt.push_back(
				sqrt(covariance(Acts::eFreeTime, Acts::eFreeTime)));
			m_err_eDir0_smt.push_back(
				sqrt(covariance(Acts::eFreeDir0, Acts::eFreeDir0)));
			m_err_eDir1_smt.push_back(
				sqrt(covariance(Acts::eFreeDir1, Acts::eFreeDir1)));
			...
			// Smoothed parameter pull
			//~ m_pull_ePos0_smt.push_back((parameters[Acts::eFreePos0] - truthHit.position().x())/
				//~ sqrt(covariance(Acts::eFreePos0, Acts::eFreePos0)));
			//~ m_pull_ePos1_smt.push_back((parameters[Acts::eFreePos1] - truthHit.position().y())/
				//~ sqrt(covariance(Acts::eFreePos1, Acts::eFreePos1)));
			//~ m_pull_ePos2_smt.push_back((parameters[Acts::eFreePos2] - truthHit.position().z())/
				//~ sqrt(covariance(Acts::eFreePos2, Acts::eFreePos2)));
			m_pull_ePos0_smt.push_back((residual.x())/
					sqrt(resCov(Acts::eFreePos0, Acts::eFreePos0)));
			m_pull_ePos1_smt.push_back((residual.y())/
					sqrt(resCov(Acts::eFreePos1, Acts::eFreePos1)));
			m_pull_ePos2_smt.push_back((residual.z())/
				sqrt(resCov(Acts::eFreePos2, Acts::eFreePos2)));
				
			m_pull_eT_smt.push_back((parameters[Acts::eFreeTime] - truthHit.time())/
				sqrt(covariance(Acts::eFreeTime, Acts::eFreeTime)));
			m_pull_eDir0_smt.push_back((parameters[Acts::eFreeDir0] - truthHit.unitDirection().x())/
				sqrt(covariance(Acts::eFreeDir0, Acts::eFreeDir0)));
			m_pull_eDir1_smt.push_back((parameters[Acts::eFreeDir1] - truthHit.unitDirection().y())/
				sqrt(covariance(Acts::eFreeDir1, Acts::eFreeDir1)));
			m_pull_eDir2_smt.push_back((parameters[Acts::eFreeDir2] - truthHit.unitDirection().z())/
				sqrt(covariance(Acts::eFreeDir2, Acts::eFreeDir2)));
			m_pull_eQOP_smt.push_back((parameters[Acts::eFreeQOverP] - m_t_charge / truthHit.momentum4Before().template head<3>().norm())/
				sqrt(covariance(Acts::eFreeQOverP, Acts::eFreeQOverP)));
		  } else {
			// push default values if no smoothed parameter
			m_ePos0_smt.push_back(-99.);
			m_ePos1_smt.push_back(-99.);
			m_ePos2_smt.push_back(-99.);
			m_eT_smt.push_back(-99.);
			...
			m_pull_eT_smt.push_back(-99.);
			m_pull_eDir0_smt.push_back(-99.);
			m_pull_eDir1_smt.push_back(-99.);
			m_pull_eDir2_smt.push_back(-99.);
			m_pull_eQOP_smt.push_back(-99.);
			m_x_smt.push_back(-99.);
			m_y_smt.push_back(-99.);
			m_z_smt.push_back(-99.);
    ...
    m_res_ePos0_prt.clear();
    m_res_ePos1_prt.clear();
    m_res_ePos2_prt.clear();
    m_res_eDir0_prt.clear();
    m_res_eDir1_prt.clear();
    m_res_eDir2_prt.clear();
    ...
    m_err_ePos0_prt.clear();
    m_err_ePos1_prt.clear();
    m_err_ePos2_prt.clear();
    m_err_eDir0_prt.clear();
    m_err_eDir1_prt.clear();
    m_err_eDir2_prt.clear();
    ...
    m_pull_ePos0_prt.clear();
    m_pull_ePos1_prt.clear();
    m_pull_ePos2_prt.clear();
    m_pull_eDir0_prt.clear();
    m_pull_eDir1_prt.clear();
    m_pull_eDir2_prt.clear();
    ...
    m_res_ePos0_flt.clear();
    m_res_ePos1_flt.clear();
    m_res_ePos2_flt.clear();
    m_res_eDir0_flt.clear();
    m_res_eDir1_flt.clear();
    m_res_eDir2_flt.clear();
    ...
    m_err_ePos0_flt.clear();
    m_err_ePos1_flt.clear();
    m_err_ePos2_flt.clear();
    m_err_eDir0_flt.clear();
    m_err_eDir1_flt.clear();
    m_err_eDir2_flt.clear();
    ...
    m_pull_ePos0_flt.clear();
    m_pull_ePos1_flt.clear();
    m_pull_ePos2_flt.clear();
    m_pull_eDir0_flt.clear();
    m_pull_eDir1_flt.clear();
    m_pull_eDir2_flt.clear();
    ...
    m_res_ePos0_smt.clear();
    m_res_ePos1_smt.clear();
    m_res_ePos2_smt.clear();
    ...
    m_err_ePos0_smt.clear();
    m_err_ePos1_smt.clear();
    m_err_ePos2_smt.clear();
    ...
    m_pull_ePos0_smt.clear();
    m_pull_ePos1_smt.clear();
    m_pull_ePos2_smt.clear();
    m_pull_eDir0_smt.clear();
    m_pull_eDir1_smt.clear();
    m_pull_eDir2_smt.clear();
