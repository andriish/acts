...
namespace MomentumParametrisations{

static constexpr uint32_t s_MaxValue = UINT32_MAX;
using EventCollection = std::vector<ActsExamples::EventFraction>;
...
template <unsigned int length_t>
using EigenspaceComponents = std::tuple<Vector<length_t>, Matrix<length_t>, Vector<length_t>>;
...
EventProperties
prepateMomenta(const EventCollection& events, unsigned int multiplicity, bool soft);

/// @brief This method builds the distributions of each generation of a final state
///
/// @param [in] events The storage of a property with fixed multiplicity and interaction type
/// @param [in] nBins Number of bins in the histogram
///
/// @return Vector containing the distributions for each generation
ProbabilityDistributions
buildMomPerMult(const EventProperties& events, unsigned int nBins);

/// @brief Transform probability distribution entries to standard normal distribution entries
///
...
EventProperties
convertEventToGaussian(const ProbabilityDistributions& histos, const EventProperties& events);

/// @brief Calculate the mean and the covariance matrix of a final state property
///
...
///
/// @return Tuple containing the eigenvalues, eigenvectors and the mean in eigenspace
template<unsigned int multiplicity_t>
EigenspaceComponents<multiplicity_t>
calculateEigenspace(const Vector<multiplicity_t>& mean, const Matrix<multiplicity_t>& covariance)
{
...
/// @brief This method builds decomposed cumulative probability distributions out of a vector of proability distributions
///
/// @param [in] histos Vector of probability distributions
///
/// @return Vector containing the decomposed cumulative probability distributions
std::vector<CumulativeDistribution>
buildMaps(const ProbabilityDistributions& histos);

/// @brief This function calculates all components required for simulating final state momenta
///
