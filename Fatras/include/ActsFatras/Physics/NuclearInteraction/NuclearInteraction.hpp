...
/// required parametrization needs to be set and is not provided by default.
///
  ...
  std::vector<Particle> operator()(generator_t& generator,
    ...
    for (const auto& particleParametrization : multiParticleParameterisation) {
      if (particleParametrization.first == particle.pdg()) {
        ...
        // Get the parameters
        const detail::Parametrization& singleParticleParametrization =
            particleParametrization.second;
        const detail::Parameters& parametrization = findParameters(
            uniformDistribution(generator), singleParticleParametrization,
        ...
        if (particle.pathLimitL0() ==
          ...
          const auto& distribution =
              parametrization.nuclearInteractionProbability;
        ...
        if (particle.pathInL0() >= particle.pathLimitL0()) {
          ...
          const bool softInteraction =
              softInteraction(normalDistribution(generator),
                              parametrization.softInteractionProbability);

          // Get the final state multiplicity
          const unsigned int multiplicity = finalStateMultiplicity(
              uniformDistribution(generator),
              softInteraction ? parametrization.softMultiplicity
                              : parametrization.hardMultiplicity);

          // Get the particle types
          const std::vector<int> pdgIds =
              samplePdgIds(generator, parametrization.pdgMap, multiplicity,
                           particle.pdg(), softInteraction);

          // Get the multiplicity
          const std::vector<
              detail::Parameters::ParametersWithFixedMultiplicity>&
              parametrizationOfType =
                  softInteraction ? parametrization.softKinematicParameters
                                  : parametrization.hardKinematicParameters;
          const detail::Parameters::ParametersWithFixedMultiplicity&
              parametrizationOfMultiplicity =
                  parametrizationOfType[multiplicity];

          // Get the kinematics
          const auto kinematics = sampleKinematics(
              generator, parametrizationOfMultiplicity,
              parametrization.momentum, multiplicity, softInteraction);
...
Acts::ActsDynamicVector NuclearInteraction::sampleInvariantMasses(
    generator_t& generator,
    const detail::Parameters::ParametersWithFixedMultiplicity& parametrization)
    const {
  ...
  for (unsigned int i = 0; i < size; i++) {
    ...
    std::normal_distribution<float> dist{parametrization.meanInvariantMass[i],
                                         sqrtf(variance)};
  ...
  for (unsigned int i = 0; i < size; i++) {
    ...
    parameters[i] = sampleContinuousValues(
        cdf, parametrization.invariantMassDistributions[i]);
  }
  return parameters;
}
...
Acts::ActsDynamicVector NuclearInteraction::sampleMomenta(
  ...
  for (unsigned int i = 0; i < size + 1; i++) {
    ...
    std::normal_distribution<float> dist{parametrization.meanMomentum[i],
                                         sqrtf(variance)};
    parameters[i] = dist(generator);
  }
  // Transform to multivariate normal distribution
  ...
  for (unsigned int i = 0; i < size + 1; i++) {
    ...
    parameters[i] =
        sampleContinuousValues(cdf, parametrization.momentumDistributions[i]);
  }
